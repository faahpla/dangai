...
/**
 * Todo whisper-cli vivo, para que exista alguem capaz de mata-los.
 *
 * Em 12/09/2026 a Biblioteca disparou a leitura do roteiro varias vezes
 * seguidas. Cada chamada subia um whisper-cli proprio, cada um carregando o
 * modelo inteiro na memoria: 24 processos, 20,4 GB de RAM, a maquina no chao.
 * Fechar a janela nao adiantava -- nada aqui sabia que eles existiam, e eles
 * sobreviviam ao app. So o Gerenciador de Tarefas resolvia.
 *
 * O processo e filho DESTE processo: quem o criou tem a obrigacao de saber
 * dele e de leva-lo junto ao sair.
 */
const vivos = new Set<ReturnType<typeof spawn>>()

function registrar(child: ReturnType<typeof spawn>): void {
  vivos.add(child)
 ...
 */
/**
 * As transcricoes em andamento, por audio+modelo+vocabulario.
 *
 * Duas chamadas para o MESMO audio nao sao duas tarefas: sao duas pessoas
 * esperando a mesma resposta. Antes cada uma subia o seu whisper-cli para
 * chegar, minutos depois, exatamente ao mesmo texto -- e era isso que
 * multiplicava 850 MB por quantas vezes a tela pedisse.
 */
const emAndamento = new Map<
  string,
  { promessa: Promise<Transcript | null>; ouvintes: Set<WhisperProgress> }
>()

/**
 * Um whisper-cli de cada vez, em fila.
 *
 * Rede para o caso que o compartilhamento nao cobre: audios diferentes pedidos
 * ao mesmo tempo. Nao custa velocidade -- o whisper.cpp ja usa todos os nucleos
 * e dois em paralelo so disputam CPU enquanto dobram a memoria.
 */
let fila: Promise<unknown> = Promise.resolve()

function enfileirar<T>(tarefa: () => Promise<T>): Promise<T> {
  // O catch mantem a fila andando: uma transcricao que falha nao pode travar
  // para sempre a proxima.
  const proxima = fila.catch(() => {}).then(tarefa)
  fila = proxima.catch(() => {})
  return proxima
}
...
export function transcribe(
  ...
  const chave = JSON.stringify([model, audioPath, [...vocabulary].sort()])

  const jaPedido = emAndamento.get(chave)
  if (jaPedido) {
    // Entra de carona: recebe o mesmo andamento e a mesma resposta.
    jaPedido.ouvintes.add(onProgress)
    return jaPedido.promessa.finally(() => jaPedido.ouvintes.delete(onProgress))
  }

  const ouvintes = new Set<WhisperProgress>([onProgress])
  const avisarTodos: WhisperProgress = (message) => {
